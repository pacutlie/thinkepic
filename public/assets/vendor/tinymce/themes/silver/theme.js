...
(function () {
  ...
  const forbid = (key, message) =>
    field$1(
      ...
      key,
      asOption(),
      value$3((_v) => SimpleResult.serror("The field: " + key + " is forbidden. " + message))
    );
  ...
  const all$2 =
    (handlers, f) =>
    (...args) =>
      foldl(handlers, (acc, handler) => acc && f(handler).apply(undefined, args), true);
  const any =
    (handlers, f) =>
    (...args) =>
      foldl(handlers, (acc, handler) => acc || f(handler).apply(undefined, args), false);
  ...
  const schema$z = objOf([
    field$1("dom", "dom", required$2(), objOf([required$1("tag"), defaulted("styles", {}), defaulted("classes", []), defaulted("attributes", {}), option$3("value"), option$3("innerHtml")])),
  ...
  const adt$a = Adt.generate([{ southeast: [] }, { southwest: [] }, { northeast: [] }, { northwest: [] }, { south: [] }, { north: [] }, { east: [] }, { west: [] }]);
  ...
  const adt$2 = Adt.generate([{ required: ["data"] }, { external: ["data"] }, { optional: ["data"] }, { group: ["data"] }]);
  ...
  const schemas = (parts) =>
    bind$3(parts, (part) =>
      part
        ...
        .map((data) => requiredObjOf(data.name, data.schema.concat([snapshot(original())])))
        .toArray()
    );
    ...
    const schema = () =>
      infoSchema.concat([
        ...
        defaultedOf(
          "focusInside",
          "onFocus",
          valueOf((val) => (contains$2(["onFocus", "onEnterOrSpace", "onApi"], val) ? Result.value(val) : Result.error("Invalid value for focusInside")))
        ),
        output$1("handler", me),
  ...
  const create$2 = (cyclicField) => {
    const schema = [option$3("onEscape"), option$3("onEnter"), defaulted("selector", '[data-alloy-tabstop="true"]:not(:disabled)'), defaulted("firstTabstop", 0), defaulted("useTabstopAt", always), option$3("visibilitySelector")].concat([
      cyclicField,
    ]);
  ...
  const doMove$2 = (cycle) => (element, focused, gridConfig, gridState) =>
    locateVisible(element, focused, gridConfig.selector).bind((identified) =>
      cycle(identified.candidates, identified.index, gridState.getNumRows().getOr(gridConfig.initSize.numRows), gridState.getNumColumns().getOr(gridConfig.initSize.numColumns))
    );
  ...
  const doMove$1 = (movement) => (component, simulatedEvent, flowConfig, flowState) =>
    movement(component, simulatedEvent, flowConfig, flowState).bind(() => (flowConfig.executeOnMove ? execute$2(component, simulatedEvent, flowConfig) : Optional.some(true)));
  ...
  const make$6 = (detail, _rawUiSpec) => {
    ...
    const keyOnItem = (f) => (container, simulatedEvent) => {
      return closest$1(simulatedEvent.getSource(), `.${detail.markers.item}`).bind((target) =>
        container
          .getSystem()
          .getByDom(target)
          .toOptional()
  ...
  const cardTextFields = [type, text$1, optionalName, defaultedArrayOf("classes", ["tox-collection__item-label"], string)];
  ...
  const colorSwatchFields = [defaultedObjOf("initData", {}, [defaultedBoolean("allowCustomColors", true), defaultedString("storageKey", "default"), optionArrayOf("colors", anyValue())])].concat(baseFields);
  ...
  const renderCommonItem = (spec, structure, itemResponse, providersBackstage) => {
    ...
    return {
      ...
      itemBehaviours: derive$1(
        [
          config("item-events", [onMenuItemExecute(spec, itemResponse), onControlAttached(spec, editorOffCell), onControlDetached(spec, editorOffCell)]),
  ...
  const renderNormalItemStructure = (info, providersBackstage, renderIcons, fallbackIcon) => {
    ...
    const textRender = Optional.from(info.meta).fold(
      () => renderText,
      (meta) => (has$2(meta, "style") ? curry(renderStyledText, meta.style) : renderText)
    );
  ...
  const getCacheForId = (id) =>
    get$g(cacheStorage, id).getOrThunk(() => {
  ...
  const getCurrentColors = (id) =>
    map$2(getCacheForId(id).state(), (color) => ({
      type: "choiceitem",
      text: color,
      icon: "checkmark",
      value: color,
  ...
  const createChoiceItems = (items, onItemValueHandler, columns, itemPresets, itemResponse, select, providersBackstage) =>
    cat(
      map$2(items, (item) => {
  ...
  const getColorItems = (spec, backstage) => {
    ...
    return spec.initData.colors.fold(
      () => getColors$1(backstage.colorinput.getColors(spec.initData.storageKey), spec.initData.storageKey, useCustomColors),
  ...
  const makeCell = (row, col, labelId) => {
    ...
    return build$1({
      ...
      behaviours: derive$1([
        config("insert-table-picker-cell", [run$1(mouseover(), Focusing.focus), run$1(execute$5(), emitExecute), run$1(click(), onClick), run$1(tap(), onClick)]),
        Toggling.config({
  ...
  const setup$d = (container, rawSettings) => {
    ...
    const simpleEvents = map$2(pointerEvents.concat(["selectstart", "input", "contextmenu", "change", "transitionend", "transitioncancel", "drag", "dragstart", "dragend", "dragenter", "dragleave", "dragover", "drop", "keyup"]), (type) =>
  ...
  const adt$1 = Adt.generate([{ stopped: [] }, { resume: ["element"] }, { complete: [] }]);
  ...
  const doTriggerOnUntilStopped = (lookup, eventType, rawEvent, rawTarget, source, logger) =>
    doTriggerHandler(lookup, eventType, rawEvent, rawTarget, source, logger).fold(always, (parent) => doTriggerOnUntilStopped(lookup, eventType, rawEvent, parent, source, logger), never);
  ...
  var InvalidateSchema = [
    ...
    optionObjOf("notify", [defaulted("aria", "alert"), defaulted("getContainer", Optional.none), defaulted("validHtml", ""), onHandler("onValid"), onHandler("onInvalid"), onHandler("onValidate")]),
  ...
  const makeFactory = (translate, getClass) => {
    const factory = (detail) => {
      ...
      return {
        ...
        behaviours: derive$1([
          config("colour-picker-events", [run$1(fieldsUpdate, onFieldsUpdate()), run$1(paletteUpdate, onPaletteUpdate()), run$1(sliderUpdate, onSliderUpdate())]),
  ...
  const renderLabel$1 = (spec, backstageShared) => {
    ...
    return {
      ...
      behaviours: derive$1([ComposingConfigs.self(), Replacing.config({}), RepresentingConfigs.domHtml(Optional.none()), Keying.config({ mode: "acyclic" })]),
    };
  };
  ...
  const renderSizeInput = (spec, providersBackstage) => {
    ...
    const pLock = FormCoupledInputs.parts.lock({
      ...
      buttonBehaviours: derive$1([Disabling.config({ disabled: () => !spec.enabled || providersBackstage.isDisabled() }), receivingConfig(), Tabstopping.config({})]),
    });
  ...
  const schema$g = constant$1(
    [
      ...
      field("typeaheadBehaviours", [Focusing, Representing, Streaming, Keying, Toggling, Coupling]),
  ...
  const renderCommonSpec = (spec, actionOpt, extraBehaviours = [], dom, components, providersBackstage) => {
    ...
    const common = {
      buttonBehaviours: derive$1(
        [DisablingConfigs.button(() => !spec.enabled || providersBackstage.isDisabled()), receivingConfig(), Tabstopping.config({}), config("button press", [preventDefault("click"), preventDefault("mousedown")])].concat(extraBehaviours)
      ),
      eventOrder: {
  ...
  const morphAdt = Adt.generate([{ static: [] }, { absolute: ["positionCss"] }, { fixed: ["positionCss"] }]);
        ...
        case "absolute":
          ...
          return Optional.some(
            morphAdt.absolute(
              NuPositionCss(
                "absolute",
                get$g(position.style, "left").map((_left) => box.x - offsetBox.x),
  ...
  const renderThrobber = (spec) => ({
    ...
    behaviours: derive$1([Replacing.config({}), Blocking.config({ focus: false }), Composing.config({ find: (comp) => head(comp.components()) })]),
    components: [],
  });
  ...
  const schema$b = constant$1(
    [markers$1(["overflowToggledClass"]), optionFunction("getOverflowBounds"), required$1("lazySink"), customField("overflowGroups", () => Cell([])), onHandler("onOpened"), onHandler("onClosed")].concat(schema$c())
  );
  ...
  const normalButtonFields = [requiredStringEnum("type", ["button"]), text$1, defaultedStringEnum("buttonType", "secondary", ["primary", "secondary"]), requiredFunction("onAction")];
  const viewButtonSchema = choose$1("type", { button: normalButtonFields });

  const viewSchema = objOf([defaultedArrayOf("buttons", [], viewButtonSchema), requiredFunction("onShow"), requiredFunction("onHide")]);
  ...
  const makeSlotContainer = (viewConfigs, providers) =>
    SlotContainer.sketch((parts) => ({
      dom: {
        tag: "div",
  ...
  const factory$6 = (detail, components, _spec) => {
    ...
    const apis = {
      ...
      toggleView: (comp, name) => {
        return parts$a.getPart(comp, detail, "viewWrapper").exists((wrapper) =>
          ViewWrapper.toggleView(
            wrapper,
            () => apis.showMainView(comp),
            () => apis.hideMainView(comp),
            name
          )
        );
      },
  ...
  const systemStackFonts = ["-apple-system", "Segoe UI", "DM Sans", "Helvetica Neue", "sans-serif"];
              ...
              .map((r) =>
                Reflecting.config({
                  ...
                  renderComponents: (data, _state) =>
                    componentRenderPipeline([data.icon.map((iconName) => renderIconFromPack(iconName, providersBackstage.icons)), data.text.map((text) => renderLabel(text, "tox-tbtn", providersBackstage))]),
                })
              )
              .toArray()
          )
  ...
  const renderToolbarToggleButtonWith = (spec, providersBackstage, bonusEvents) =>
    renderCommonToolbarButton(
      spec,
      {
  ...
  const renderSplitButton = (spec, sharedBackstage) => {
    ...
    return SplitDropdown.sketch({
      ...
      splitDropdownBehaviours: derive$1([
        ...
        receivingConfig(),
        config("split-dropdown-events", [run$1(focusButtonEvent, Focusing.focus), onControlAttached(specialisation, editorOffCell), onControlDetached(specialisation, editorOffCell)]),
        Unselecting.config({}),
      ]),
      eventOrder: {
  ...
  const lookupButton = (editor, buttons, toolbarItem, allowToolbarGroups, backstage, prefixes) =>
    get$g(buttons, toolbarItem.toLowerCase())
      .orThunk(() => prefixes.bind((ps) => findMap(ps, (prefix) => get$g(buttons, prefix + toolbarItem.toLowerCase()))))
      .fold(
        () => get$g(bespokeButtons, toolbarItem.toLowerCase()).map((r) => r(editor, backstage)),
        (spec) => {
  ...
  const LazyUiReferences = () => {
    ...
    const lazyGetInOuterOrDie = (label, f) => () =>
      mainUi
        .get()
        .bind((oc) => f(oc.outerContainer))
        .getOrDie(`Could not find ${label} element in OuterContainer`);
    return {
      dialogUi,
      popupUi,
      mainUi,
  ...
  const onSetupUndoRedoState = (editor, type) =>
    onSetupEvent(editor, "Undo Redo AddUndo TypingUndo ClearUndos SwitchMode", (api) => {
  ...
  const getCurrentCoord = (target) =>
    lift3(getRaw(target, "left"), getRaw(target, "top"), getRaw(target, "position"), (left, top, position) => {
  ...
  const schema$5 = [...schema$6, output$1("dragger", { handlers: handlers(events$2) })];
  ...
  const schema$4 = [...schema$6, output$1("dragger", { handlers: handlers(events$1) })];
  ...
  const schema$3 = [...schema$6, output$1("dragger", { handlers: handlers(events) })];
  ...
  const setup$3 = (editor) => {
    ...
    const renderMainUi = () => {
      ...
      const editorContainer = OuterContainer.parts.editorContainer({
        components: flatten([editorComponents, isInline ? [] : statusbar.toArray()]),
      });
      ...
      const outerContainer = build$1(
        OuterContainer.sketch({
          ...
          components: [editorContainer, ...(isInline ? [] : [partViewWrapper]), partThrobber],
          behaviours: derive$1([
            receivingConfig(),
  ...
  const alertBannerFields = [type, text$1, requiredStringEnum("level", ["info", "warn", "error", "success"]), icon, defaulted("url", "")];
  const alertBannerSchema = objOf(alertBannerFields);

  const createBarFields = (itemsField) => [type, itemsField];

  const buttonFields = [type, text$1, enabled, generatedName("button"), optionalIcon, borderless, optionStringEnum("buttonType", ["primary", "secondary", "toolbar"]), primary];
  ...
  const collectionDataProcessor = arrOfObj([value$1, text$1, icon]);
  ...
  const selectBoxFields = formComponentWithLabelFields.concat([requiredArrayOfObj("items", [text$1, value$1]), defaultedNumber("size", 1), enabled]);
      ...
      if (ALLOW_DATA_ATTR && !FORBID_ATTR[lcName] && regExpTest(DATA_ATTR$1, lcName));
        ...
        if (
          (_basicCustomElementTest(lcTag) &&
            ((CUSTOM_ELEMENT_HANDLING.tagNameCheck instanceof RegExp && regExpTest(CUSTOM_ELEMENT_HANDLING.tagNameCheck, lcTag)) ||
              (CUSTOM_ELEMENT_HANDLING.tagNameCheck instanceof Function && CUSTOM_ELEMENT_HANDLING.tagNameCheck(lcTag))) &&
            ((CUSTOM_ELEMENT_HANDLING.attributeNameCheck instanceof RegExp && regExpTest(CUSTOM_ELEMENT_HANDLING.attributeNameCheck, lcName)) ||
              (CUSTOM_ELEMENT_HANDLING.attributeNameCheck instanceof Function && CUSTOM_ELEMENT_HANDLING.attributeNameCheck(lcName)))) ||
          (lcName === "is" &&
            CUSTOM_ELEMENT_HANDLING.allowCustomizedBuiltInElements &&
            ((CUSTOM_ELEMENT_HANDLING.tagNameCheck instanceof RegExp && regExpTest(CUSTOM_ELEMENT_HANDLING.tagNameCheck, value)) || (CUSTOM_ELEMENT_HANDLING.tagNameCheck instanceof Function && CUSTOM_ELEMENT_HANDLING.tagNameCheck(value))))
        );
        else {
          return false;
        }
  ...
  const validateData$1 = (access, data) => {
    ...
    return Reflecting.getState(root)
      .get()
      .map((dialogState) => getOrDie(asRaw("data", dialogState.dataValidator, data)))
      .getOr(data);
  };
